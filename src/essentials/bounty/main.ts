...
import { BountyData, BountyList } from "../../interfaces";
...
export class BountyBoard {
  private moneyProperty = "ess:bounty_board";

  private getWorldBountyData(): BountyData[] {
  ...
  private saveWorldBountyData(data: BountyData[]) {
  ...
  get(playerNameTag: string): BountyList[] {
  ...
  update(playerNameTag: string, newBountyList: BountyList, itemStack: ItemStack) {
  ...
  set(playerNameTag: string, newBountyList: BountyList[]) {
    const data = this.getWorldBountyData();
    const index = data.findIndex(d => getActualName(d.nameTag) === playerNameTag);
    if (index === -1) return;
