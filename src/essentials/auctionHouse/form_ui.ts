...
import { AuctionSold } from "../../interfaces";
...
function auctionHouseEdit(player: Player, soldItem: ItemStack, auctionSold: AuctionSold) {
  ...
  form.show(player).then(res => {
    ...
    const newPrice = parseFloat(res.formValues[1] as string);
    ...
    if (remove) {
      ...
      auctionHouse.remove(player.nameTag, auctionSold.id);
    } else if (newPrice !== auctionSold.price) {
      auctionHouse.remove(player.nameTag, auctionSold.id);
      auctionHouse.add(player.nameTag, newPrice, auctionSold.amount, soldItem);
      auctionHouseSeller(player);
    }
  });
}

export function auctionHouseCatalog(player: Player) {
  const auctionData = new AuctionHouse().getAll();
  const auctionItems: { item: ItemStack; seller: string, data: any }[] = [];
...
export function auctionHouseSellItem(player: Player) {
  ...
  form.show(player).then(res => {
    ...
    if (!isNaN(price) && !isNaN(amount) && price > 0 && amount > 0 ) {
      let confirm = new MessageFormData()
        ...
        if (res.selection === 0) auctionHouseSellOption(player, itemStack);
        if (res.selection === 1) {
          ...
          new AuctionHouse().add(player.nameTag, price, amount, soldItem)
