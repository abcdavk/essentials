...
export function handleAddFriendUI(player: Player, block: Block, dimension: Dimension, protectionData: ProtectionData) {
  ...
  players.forEach(p => {
    if (getActualName(p.nameTag) !== getActualName(player.nameTag)) playerList.push(getActualName(p.nameTag));
  })
  ...
  form.show(player).then(res => {
    if (res.formValues) {
      ...
      friendList.push({
        ...
        allow_teleport_to_plot: res.formValues[9] as boolean,
      });
