...
world.afterEvents.worldLoad.subscribe(() => {
  const newPlayerInventoryListString = world.getDynamicProperty("playerInventoryList") as string;
  const newPlayerEquipmentListString = world.getDynamicProperty("playerEquipmentList") as string;
  const newPlayerXpListString = world.getDynamicProperty("playerXpList") as string;
...
function resetXp(newXp: number, player: Player) {
  if (!player || newXp === void 0 || newXp < 0)
    return;
  player.addExperience(newXp);
}
function resetEquipment(equipment: [ItemStack], player: Player) {
  if (!player || !equipment) {
    return;
  }
  const playerEquipment = player.getComponent(
...
function resetInventory(inventory: [ ItemStack ], player: Player) {
  if (!player || !inventory) {
...
system.afterEvents.scriptEventReceive.subscribe((event) => {
  ...
  switch (command.toLowerCase()) {
    case "togglekeepondeath":
      toggleKeepOnDeath(target as Player);
      break;
  }
});
function toggleKeepOnDeath(player: Player) {
  player.hasTag("keep_on_death") ? player.removeTag("keep_on_death") : player.addTag("keep_on_death");
  player.sendMessage(
    `Keep on death ${player.hasTag("keep_on_death") ? "enabled" : "disabled"} for player ${player.name}`
  );
}
