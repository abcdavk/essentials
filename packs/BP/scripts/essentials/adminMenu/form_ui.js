...
export function adminMenuMainUI(player) {
    ...
    form.show(player).then(res => {
        ...
        if (res.selection === 4)
            adminAddPrivilage(player);
        // if (res.selection === 3) adminMenuRemoveTitle(player);
    });
}
export function adminAddPrivilage(player) {
    let has_privilage = player.hasTag("has_privilage");
    if (!has_privilage) {
    ...
    form.show(player).then(res => {
        if (res.formValues) {
            if (res.formValues[0] !== 0 && res.formValues[1] !== "") {
                ...
                const [dropdown, typeName, amount] = res.formValues;
                if (amount === undefined)
                    return;
                if (dropdown !== 0) {
...
function adminMenuAddMoney(player) {
    const players = world.getPlayers();
    let playerList = ["None"];
    players.forEach(p => {
        // if (getActualName(p.nameTag) !== getActualName(player.nameTag)) 
...
function adminMenuRemoveMoney(player) {
    const players = world.getPlayers();
    let playerList = ["None"];
    players.forEach(p => {
        // if (getActualName(p.nameTag) !== getActualName(player.nameTag)) 
