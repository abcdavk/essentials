import { EntityComponentTypes, ItemComponentTypes, system, world } from "@minecraft/server";
...
export function bountyBoardCatalog(player) {
...
function bountyBoardTake(player, seller, wantedItem, bountyList) {
                        ...
                        if (wantedItemEnchantments.length > 0) {
                            ...
                            for (const invEnchant of itemInvEnchantments) {
                                for (const wantEnchant of wantedItemEnchantments) {
                                    if (invEnchant.type === wantEnchant.type &&
...
export function bountyBoardMainUI(player) {
    let form = new ActionFormData()
...
export function bountyBoardPlaceItem(player) {
...
export function bountyBoardOwner(player) {
    ...
    for (const bounty of bountyData) {
        if (bounty.nameTag !== getActualName(player.nameTag))
            continue;
    ...
    form.show(player).then(res => {
        ...
        if (selected.data.completed) {
            let soldItem = Inventories.get(selected.data.id);
            soldItem.amount = 1;
...
function bountyBoardEdit(player, soldItem, bountyList) {
    const bountyBoard = new BountyBoard();
    const itemName = soldItem.nameTag !== undefined ? soldItem.nameTag : itemTypeIdToName(soldItem.typeId);
...
function bountyBoardPlaceOption(player, itemStack) {
        ...
        if (!isNaN(price) && !isNaN(amount) && price > 0 && amount > 0) {
            ...
            if (!isCanceled) {
                let confirm = new MessageFormData()
                    ...
                    if (res.selection === 0)
                        bountyBoardPlaceOption(player, itemStack);
                    if (res.selection === 1) {
