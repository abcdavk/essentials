...
function sellHandSelectItem(player) {
...
function getItemSellPrice(itemId) {
    for (const entry of itemSellPriceRegistry) {
        if (entry.includes) {
            if (itemId.includes(entry.item))
                return entry.price;
        }
        else {
            if (itemId === entry.item)
                return entry.price;
        }
    }
    return itemSelldefaultPrice;
}
function sellHandOption(player, itemStack) {
