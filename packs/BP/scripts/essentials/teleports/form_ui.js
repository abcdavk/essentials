...
export function teleportPlot(player) {
    if (!player.hasTag("ess:tp_cooldown")) {
        const playerPlot = new Protection().getAll(player.nameTag);
...
export function teleportAsk(player) {
    ...
    form.show(player).then(res => {
        if (!res.formValues)
            return;
        const [selectedIndex, typedName] = res.formValues;
